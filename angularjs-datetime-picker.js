  ...
  var datetimePickerPopup = function($locale, dateFilter){
    ...
    var linkFunc = function(scope, element, attrs, ctrl) { //jshint ignore:line
      ...
      scope.$applyAsync( function() {
        ...
        var datetime = dateFilter(elScope.$eval(attrs.ngModel), dateFormat);
        
        if(datetime){
          var info = datetime.toString().split('-');
          if(info.length > 2){
            var year = info[2].slice(0, 4);
            var month = info[1]-1;
            var day = info[0];
            var hour = info[2].slice(5, 7);
            var minute = info[2].slice(8, 10);  
          }
          ...
          var preSelectedDate = new Date(year, month, day, hour, minute, 0);
        }

      var modelController = element.find('input').controller('ngModel');
      
      scope.selectedDate = preSelectedDate ? preSelectedDate : scope.selectedDate;
      //scope.selectedDate =  scope.selectedDate;
      

      if(scope.selectedDate == 'Invalid Date'){
        alert("Invalid Date! Please select valid date");
        modelController.$setValidity('inValidDate', true);
       }else{
       modelController.$setValidity('inValidDate', false);
       }
